...
pub(crate) mod test_macros {
    ...
    /// Create a [`TimeInterval`](super::TimeInterval) literal.
    ...
    /// Create a [`SlotMap`](super::SlotMap)s for testing.
    ///
    /// Expects `m/d/y - m/d/y` format. Time can be appended to date with `@ h:m`.
    macro_rules! slots {
        ($(
            ...
            [$($crate::slot_lit!($id: $mo0/$d0/$yr0$( @ $hr0:$m0)? - $mo1/$d1/$yr1$( @ $hr1:$m1)? $([$min_staff])? $(| $name)?)),*]
                .into_iter()
                .map(|slot| (slot.id, slot))
                .collect::<$crate::data::slot::SlotMap>()
        };

        () => {
            $crate::data::slot::SlotMap::default()
        };
    }

    /// Create a [`Task`](super::Task) for testing.
    macro_rules! task_lit {
        (
    ...
    /// Create a [`TaskMap`](super::TaskMap) for testing.
    macro_rules! tasks {
        ($(
            $id:literal: $title:literal
            $([$mo:literal/$d:literal/$yr:literal$( @ $hr:literal:$m:literal)?])?
    ...
    /// Create a [`Rule`](super::Rule) for testing.
    macro_rules! rule_lit {
        (
            $id:literal:
            $mo0:literal/$d0:literal/$yr0:literal$( @ $hr0:literal:$m0:literal)? -
            $mo1:literal/$d1:literal/$yr1:literal$( @ $hr1:literal:$m1:literal)?
    ...
    /// Create a [`RuleMap`](super::RuleMap) for testing.
    macro_rules! rules {
        ($(
            $id:literal:
            $mo0:literal/$d0:literal/$yr0:literal$( @ $hr0:literal:$m0:literal)? -
            ...
            [$($crate::rule_lit!($id: $mo0/$d0/$yr0$( @ $hr0:$m0)? - $mo1/$d1/$yr1$( @ $hr1:$m1)? | $pref)),*]
                .into_iter()
                .map(|rule| (rule.id, rule))
                .collect::<$crate::data::rule::RuleMap>()
        };

        () => {
            $crate::data::rule::RuleMap::default()
        };
    }
